...
docker build -t frugal .               # multi-stage, CGO on, distroless base
```
...
`cmd/frugal/main.go` wires everything. The server (`runServer`) starts each collector as a goroutine writing into two shared, concurrency-safe stores, then serves HTTP:
...
- **Config** (`internal/config/config.go`): JSON file (all fields optional, `Default()` supplies sane values) with env-var overrides applied *after* the file. `AWS_REGION`/`AWS_PROFILE` and other `FRUGAL_*` envs always win over file values. Poll intervals are floored (AWS ≥30s, k8s ≥5s) to prevent API-cost surprises.
...
`deploy/k8s.yaml` (server in-cluster, IRSA for CloudWatch — needs only `cloudwatch:ListMetrics` + `GetMetricData`), `deploy/agent-daemonset.yaml` (agent per EKS node), `deploy/frugal-agent.service` (systemd EC2 agent). The dashboard has **no built-in auth** — keep it behind port-forward/VPN/authenticating ingress.
